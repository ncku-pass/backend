...
namespace Application.Services
{
    public class BackstageService : IBackstageService
    {
        ...
        public async Task<BackStageAbilityAnalyzeResponse> AbilityAnalyze(BackstageCategoriesAnalyzeMessage message)
        {
            ...
            var expModels = await this._unitOfWork.Experience.Where(e => userIds.Contains(e.UserId) && !e.Name.Contains("【範例】") && catagories.Contains(e.Type)).ToListAsync();
            var usedTags = await (from combine in this._unitOfWork.Experience_Tag.Where(et => expModels.Select(e => e.Id).Contains(et.ExperienceId))
                                  join tag in this._unitOfWork.Tag.Where(t => !t.Name.Contains("【範例】"))
                                     on combine.TagId equals tag.Id
            ...
            // 4. 用字典檔整理ExpName、TagName
            Dictionary<string, int> expResult = new Dictionary<string, int>();
            Dictionary<string, int> tagNameResult = new Dictionary<string, int>();
            Dictionary<string, List<int>> tagUserResult = new Dictionary<string, List<int>>();

            foreach (var exp in expModels)
            {
                var key = exp.Name + "," + exp.Type;
                expResult.TryGetValue(key, out int count);
                expResult[key] = count + 1;
            }

            foreach (var tag in usedTags)
            {
                tagNameResult.TryGetValue(tag.Name, out int count);
                tagNameResult[tag.Name] = count + 1;

                if (!tagUserResult.ContainsKey(tag.Name))
                {
                    tagUserResult.Add(tag.Name, new List<int>() { tag.UserId });
                }
                else
                {
                    tagUserResult[tag.Name].Add(tag.UserId);
                }
            }
            // 移除重複User
            tagUserResult = tagUserResult.ToDictionary(kvp => kvp.Key, kvp => kvp.Value.Distinct().ToList());
